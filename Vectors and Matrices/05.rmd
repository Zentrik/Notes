...
\begin{align*}
    \det M &= [\underline{C}_1, \ldots, \underline{C}_{a - 1}, \underline{C}_a, \underline{C}_{a + 1}, \ldots, \underline{C}_n] \\
    &= [\underline{C}_1, \ldots, \underline{C}_{a - 1}, \sum_i M_{ia} \underline{e}_i, \underline{C}_{a + 1}, \ldots, \underline{C}_n] \\
    &= \sum_i M_{ia} \Delta_{ia} \hspace{0.5cm} \text{(no sum over $a$)} \\
    \text{where the } &\emph{cofactor } \Delta_{ia} \text{ is defined by} \\
    \Delta_{ia} &= [\underline{C}_1, \underline{C}_2, \ldots, \underline{C}_{a- 1}, \underline{e}_i, \underline{C}_{a + 1}, \ldots, \underline{C}_n] \\
...
\begin{align*}
    [\underline{C}_1, \ldots, \underline{C}_{a - 1}, \underline{C}_b, \underline{a + 1}, \ldots, \underline{C_n}] &= \sum_i M_{ib} \delta_{ia} \\
    &= \begin{cases}
        \det M & \text{if } a = b \\
        0 & \text{if } a \neq b
    \end{cases} \\
...
\begin{align*}
    ...
    \text{i.e. } A_{11} x_1 + \ldots + A_{1n} x_n &= b_1 \\
    &\;\;\vdots \\
...
\begin{align*}
    A \underline{u} &= \underline{0} \emph{ homogenous problem}.
...
If $\underline{u}_1, \ldots, \underline{u}_k$ is a basis for $\ker A$ then general solution of homogenous problem is $\underline{u} = \sum_{i=1}^{k} \lambda_i \underline{u}_i$.
...
```{r 05-sheaf, echo = FALSE, fig.cap = "looking *along* line of intersection of planes", fig.align="center", out.height="300"} 
...
```{r 05-rank-1, echo = FALSE, fig.cap = "Three identical planes", fig.align="center", out.height="300"} 
...
```{r 05-replace, echo = FALSE, fig.cap = "", fig.align="center", out.height="300"} 
...
Note $x_{r + 1}, \ldots, x_n$ are undetermined \
$d_{r + 1}, \ldots, d_m = 0$ else no solution \
and if this is satisfied then $x_1, \ldots, x_r$ can be determined successively.\
$r = \operatorname{rank} M = \operatorname{rank} A$.\
If $n = m$ then $\det A = \pm det M$ and if $r = n = m$ then $\det M = M_{11} \ldots M_{rr} \neq 0 \implies A$ and $M$ are invertible. \
