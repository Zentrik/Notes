...
\begin{definition}[Divides]
	An element $a \in R$ \vocab{divides} $b \in R$, written $a \mid b$, if there exists $c \in R$ such that $b = ac$.
	Equivalently, $(b) \subseteq (a)$.
\end{definition}

\begin{definition}[Associates]
	Two elements $a, b \in R$ are \vocab{associates} if $a = bc$ where $c$ is a unit.
	Informally, the two elements differ by multiplication by a unit.
	Equivalently, $(a) = (b)$.
\end{definition} 

\begin{definition}
	An element $r \in R$ is \vocab{irreducible} if $r \neq 0$ is not a unit, and $r = ab$ implies $a$ or $b$ is a unit.
\end{definition}

\begin{definition}[Prime]
	An element $r \in R$ is \vocab{prime} if $r \neq 0$ is not a unit and \underline{$r \mid ab$ implies $r \mid a$ or $r \mid b$}.
\end{definition} 
...
\begin{definition}[Principal Ideal Domain]
	An integral domain $R$ is a \vocab{principal ideal domain} (PID) if all ideals are principal ideals.
	In other words, for all ideals $I \triangleleft R$, there exists an element $r$ such that $I = (r)$.
\end{definition}

\begin{example}
	$\mathbb Z$ is a principal ideal domain by \Cref{lem:8.3}.
\end{example}
...
\begin{remark}
	\begin{enumerate}
		\item The converse direction doesn't depend on $R$ being a PID only that it's an integral domain.
		\item If $R$ is a PID and $0 \neq r \in R$.
		Then, $(r)$ is maximal iff $r$ is irreducible (\Cref{lem:10.4}), which is true iff $r$ is prime (\Cref{lem:10.2,prp:10.3}), which is equivalent to the fact that $(r)$ is prime (\Cref{lem:10.1}).
		Hence, the maximal ideals are the nonzero prime ideals.
	\end{enumerate} 
\end{remark}

\begin{definition}[Euclidean Domain]
	An integral domain is a \vocab{Euclidean domain} if there exists a function $\varphi \colon R \setminus \qty{0} \to \mathbb Z_{\geq 0}$ such that, for all $a, b \in R$.
	\begin{enumerate}
		\item If $a \mid b$ then $\varphi(a) \leq \varphi(b)$;
		\item If $b \neq 0$ then $\exists \; q, r \in R$ such that $a = bq + r$ and either $r = 0$ or $\varphi(r) < \varphi(b)$.
	\end{enumerate}
	Such a $\varphi$ is called a \vocab{Euclidean function}.
\end{definition}
...
\begin{example}
	...
	The second property of Euclidean domains is proven using \Cref{prp:7.1} whilst the first is easy to check.

	So $F[X]$ is a PID by \Cref{prp:10.5}
\end{example}
...
\begin{example}
	...
	So $\mathbb Z[i]$ is a PID by \Cref{prp:10.5}
\end{example} 
...
\begin{example}[Field of order $8$]
	...
	Since $\mathbb F_2[X]$ is a principal ideal domain, we have that $(f) \triangleleft \mathbb F_2[X]$ is a maximal ideal by \Cref{lem:10.4}.
...
\underline{GOAL}: Show PID $\implies$ UFD.
...
\begin{definition}[Noetherian]
	...
	Then, we say that $R$ is a \vocab{Noetherian} ring.
\end{definition}
...
\begin{proof}
	...
	But $R$ is Noetherian, so this is a contradiction to \Cref{lem:10.7}.
	So every nonzero, non-unit element is indeed a product of irreducibles.

	By \Cref{prp:10.6}, it suffices to show that every irreducible is prime.
	This has already been shown previously by \Cref{prp:10.3}.
	Hence $R$ is a unique factorisation domain.
\end{proof}
...
\begin{definition}[Common Divisors and Multiples]
	Let $R$ be an integral domain.
	\begin{enumerate}
		\item $d \in R$ is a \vocab{common divisor} of $a_1, \dots, a_n \in R$ if $d \mid a_i$ for all $i$;
		\item $d \in R$ is a \vocab{greatest common divisor} of $a_1, \dots, a_n$ if for all common divisors $d'$, we have $d' \mid d$;
		\item $m \in R$ is a \vocab{common multiple} of $a_1, \dots, a_n$ if $a_i \mid m$ for all $i$;
		\item $m \in R$ is a \vocab{least common multiple} of $a_1, \dots, a_n$ if for all common multiples $m'$, we have $m \mid m'$.
	\end{enumerate}
\end{definition}
...
\begin{definition}[Content]
	The \vocab{content} of a polynomial $f = \sum_{i=0}^n a_i X^i \in R[X]$ is $c(f) = \mathrm{gcd}\qty{a_0, \dots, a_n}$.
	This is well-defined up to multiplication by a unit.
\end{definition}

\begin{definition}[Primitive]
	We say that $f$ is \vocab{primitive} if $c(f)$ is a unit.
\end{definition} 
...
\begin{proof}
	...
	By \Cref{lem:11.4}, we have that $h \in R[X]$, and we already saw that $\deg h > 0$.
...
\begin{proof}
	...
	By \Cref{lem:11.4}, $g \mid f_1$ or $g \mid f_2$ in $R[X]$ as required.
\end{proof}

We can now prove \Cref{thm:11.1}, that polynomial rings over unique factorisation domains are unique factorisation domains.
...
\begin{proof} \label{prf:11.1}
	...
	By \Cref{prp:10.6}, it in fact suffices to show that every irreducible element of $R[X]$ is prime.
	...
	By \Cref{cor:11.3}, $f$ is prime in $R[X]$.

	Now, suppose $f$ is primitive.
	Since $f$ is irreducible in $R[X]$, we can use \nameref{lem:Gauss} to show that $f$ is irreducible in $F[X]$.
	Thus, $f$ is prime in $F[X]$, as $F[X]$ is a unique factorisation domain.
	Finally, we can see that $f$ is prime in $R[X]$ by \Cref{lem:11.5}.
\end{proof}

\begin{remark}
	By \Cref{lem:10.2}, we know that the prime elements in an integral domain are irreducible.
	This implies that the implications in the last paragraph above are in fact equivalences.
	In particular, in \nameref{lem:Gauss}, the implication is an equivalence.
\end{remark}

\begin{example}
	\Cref{thm:11.1} implies that $\mathbb Z[X]$ is a unique factorisation domain.
\end{example}

\begin{example}
	Let $R[X_1, \dots, X_n]$ be the ring of polynomials in $n$ variables.
	Define inductively $R[X_1, \dots, X_n] = R[X_1, \dots, X_{n - 1}][X_n]$.
	Applying \Cref{thm:11.1} inductively $\implies R[X_1, \dots, X_n]$ is a UFD if $R$ is.
	% We can rewrite this as $R[X_1]\dots[X_n]$, so by induction this is a unique factorisation domain if $R$ is.
\end{example} 

\subsection{Eisenstein's criterion}
\begin{proposition}[Eisenstein's Criterion] \label{prp:eisen}
...
\begin{example}
	...
	By \nameref{lem:Gauss}, $f$ is irreducible in $\mathbb Q[X]$, since $\mathbb Q$ is the field of fractions of $\mathbb Z$.
	In particular, $\faktor{\mathbb Q[X]}{(f)}$ is a field by \Cref{lem:10.4}, since the ideal $(f)$ is maximal.
\end{example}
