...
\begin{definition}[Group Actions]
	A \vocab{group action} of a group $G$ on a set $X$ is a function $\alpha \colon G \times X \to X$ satisfying
	\begin{align*}
...
\begin{definition}[Permutation Representation]
	The homomorphism $\varphi \colon G \to \Sym(X)$ defined in the above proof is called a \vocab{permutation representation} of $G$.
\end{definition}
...
\begin{definition}[Orbit, Stabiliser]
	...
	Then,
	\begin{enumerate}
		\item the \vocab{orbit} of $x \in X$ is $\Orb_G(x) = \qty{g \ast x \colon g \in G} \subseteq X$;
		\item the \vocab{stabiliser} of $x \in X$ is $G_x = \qty{g \in G \colon g \ast x = x} \leq G$.
	\end{enumerate}
\end{definition}
...
\begin{example}
	...
	The stabiliser of a subgroup $H$ is $\qty{ g \in G \colon gHg\inv = H } = N_G(H)$, called the \vocab{normaliser} of $H$ in $G$.
