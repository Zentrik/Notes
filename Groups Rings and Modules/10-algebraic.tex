...
\begin{proof}
	...
	We have that $\qty(\faktor{\mathbb Z}{p\mathbb Z})^\times$ is cyclic of order $p-1$ by \Cref{thm:9.3}.
...
\begin{remark}
	In the \Cref{thm:12.2}, if $p = a^2 + b^2$, $a+bi$ and $a-bi$ are not associate unless $p = 2$.
...
\begin{proof}
	...
	By \Cref{thm:12.2}, those norms are
	\begin{enumerate}
...
\begin{definition}[Algebraic]
	A number $\alpha \in \mathbb C$ is \vocab{algebraic} if $\alpha$ is a root of some nonzero polynomial $f \in \mathbb Q[X]$.
\end{definition}

\begin{definition}[Algebraic Integer]
	$\alpha \in \mathbb{C}$ is an \vocab{algebraic integer} if it is a root of some monic polynomial $f \in \mathbb Z[X]$.
\end{definition} 
...
\begin{notation}
	...
	Further, $R[\alpha] = \Im \varphi$ where $\varphi \colon R[X] \to S$ is the homomorphism $g(X) \mapsto g(\alpha)$\footnote{We map polynomial $g$ to $g(\alpha)$}.
\end{notation} 

\begin{definition}[Minimal Polynomial]
	Let $\alpha$ be an algebraic number.
	Consider the homomorphism $\varphi \colon \mathbb Q[X] \to \mathbb C$ where $g(X) \mapsto g(\alpha)$, $\mathbb{Q}[X] = \Im \phi$.
	...
	This unique $f$ is known as the \vocab{minimal polynomial} of $\alpha$.
\end{definition} 
...
\begin{proof}
	...
	By \Cref{lem:11.4}, $\lambda f \mid g$ in $\mathbb Z[X]$.
