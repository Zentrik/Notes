...
::: {.definition #sixteen}
A group is called *simple* if its only normal subgroups are $\{ e \}$ and $G$.
...
i. $f$ is an *injective* map if $a_1, a_2 \in A$, $f(a_1) = f(a_2) \implies a_1 = a_2$ (each element of $A$ maps to a different element of $B$).
...
::: {.proof name="1st Isomorphism Theorem"}
...
```{r 04-iso-thm, echo = FALSE, fig.cap = "", fig.align="center", out.height = "300px"} 
knitr::include_graphics("figures/04-iso-thm.png") 
```
...
::: {.lemma #thirteen}
A homomorphism $\theta : G \to H$ is injective iff $\ker \theta = \{ e_G \}$.
::: 
...
i. Let $N \trianglelefteq G, H \leq G$.
...
i. Closure, $nh, \bar{n} \bar{h} \in NH$
\begin{align*}
