...
\begin{definition}[Abstract Riemannian Metric]
	Let $V \subset \mathbb R^2$ be an open set.
	An \vocab{abstract Riemannian metric} is a smooth map from $V$ to the set of positive definite symmetric bilinear forms, given by
	\begin{align*}
...
\begin{definition}[Isometric]
	...
	They are \vocab{isometric} if there exists a diffeomorphism $f: V_1 \to V_2$ s.t.
	\begin{align*}
...
\end{align*} 
Recall that $(\dagger)$ is exactly the transformation law \Cref{lem:2.3}.
...
\begin{definition}[Riemannian Metric]
	...
	A \vocab{Riemannian metric} on $\Sigma$, usually called $g$ or $\dd{s}^2$, is a choice of Riemannian metric $g_i$ in the above sense on each $V_i$, which are compatible in the following sense.
...
\begin{definition}[Length Metric]
	...
	The \vocab{length metric} is defined by
	\begin{align*}
