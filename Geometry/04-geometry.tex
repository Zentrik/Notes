...
\begin{definition}[Gauss Map]
	...
	The \vocab{Gauss map} $n \colon \Sigma \to S^2$\footnote{$S^2 = \{x \in \mathbb{R}^3 : |x| = 1\}$} is the map $p \mapsto n(p)$, where the normal vector is defined by the orientation of $\Sigma$ and is normalised so it lies in the unit sphere.
\end{definition}
...
\begin{definition}[Gauss Curvature]
	...
	The \vocab{Gauss curvature} $\kappa \colon \Sigma \to \mathbb R$ of $\Sigma$ is the function defined by
	\begin{align*}
...
\begin{definition}[Flat]
	A smooth surface $\Sigma \subset \mathbb R^3$ with vanishing Gauss curvature everywhere on $\Sigma$ is \vocab{flat}.
\end{definition}
...
\begin{theorem}[\textit{theorema Egregium}] \label{thm:egregium}
