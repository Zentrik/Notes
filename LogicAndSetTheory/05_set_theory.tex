...
\begin{lemma}
    Every set is contained in a transitive set, i.e. $(\forall x)(\exists y)(\text{`$i$ is transitive'} \wedge x \subseteq y)$.
\end{lemma}
...
\begin{proof}
    ...
    Then $n \in \dom g$ so $\bigcup z = \bigcup f(n) = \bigcup g(n)$ by $(\ast)$ and $\bigcup g(n) = \bigcup g(n^+) \in w$, hence $a \subseteq t$.
\end{proof}
...
\end{theorem}
\begin{proof}
    Given $t_1, \dots, t_n$ and the statement $(\forall x)((\forall y)(y \in x \implies p(y)) \implies p(x))$, we want to show $(\forall x)p(x)$.
...
We say that $x$ is \vocab{regular} if $(\forall y)(x \in y \implies y \text{ has a minimal element})$.
...
\begin{proof}
    ...
    \begin{align*}
        f \text{ is a function} \wedge \dom f \text{ is transitive} \wedge (\forall x)\qty(x \in \dom f \implies f(x) = G\qty(\eval{f}_x))
    \end{align*}
...
\begin{proof}
    ...
    \end{align*}
    by $r$-induction on $x$.
    Suppose that $(\forall y r x)(\forall z \in a)(f(y) = f(z) \implies y = z)$, we have $f(x) = f(x')$, and we want to show that $x = x'$.
