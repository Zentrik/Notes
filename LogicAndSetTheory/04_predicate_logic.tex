...
\begin{definition}[Proof]
    ...
    A \vocab{proof} of $p$ from $S$ is a finite sequence $t_1, \dots, t_n$ of formulae s.t. $t_n = p$ and $\forall i \; t_i$ is an axiom, a premise or deduced from previous lines (i.e. $\exists j, x < i$ s.t. $t_x = (t_j \implies t_i)$ (MP) or $\exists j < i$ s.t. $t_i = (\forall x)t_j$, $x \in \FV(t_j)$ and $\forall k < j$ if $t_k \in S$ then $x \notin \FV(t_k)$ (Gen)). \\
