...
\begin{definition}
    ...
    A \vocab{theory} is a set of sentences, known as its \vocab{axioms}.
...
\end{enumerate}
For $S \subseteq L$ and $t \in L$, we say that $S \vdash p$, read $S$ \vocab{proves} $p$, if there exists a \vocab{proof} of $p$ from $S$, which is a finite sequence of formulae ending with $p$ s.t. each formula is a logical axiom, a hypothesis in $S$, or obtained from earlier lines by one of the deduction rules.
\begin{remark}
...
The $y_i$ in (iii) are called \vocab{parameters}.
...
\begin{definition}
    A set $S \subseteq \mathbb N$ is \vocab{definable} in the language of $\mathsf{PA}$ if there is a formula $p$ with a free variable $x$ s.t. for each $m \in \mathbb N$, $m \in S$ iff $p[m/x]$ holds in $\mathbb N$.
\end{definition}
