...
\begin{definition}
    A \vocab{total order} or \vocab{linear order} is a pair $(X, <)$ where $X$ is a set, and $<$ is a relation on $X$ such that
    \begin{itemize}
...
\begin{definition}
    A total order $(X, <)$ is a \vocab{well-ordering} if every nonempty subset $S \subseteq X$ has a least element.
    \begin{align*}
...
\end{remark}
\begin{definition}
    Total orders $X, Y$ are \vocab{isomorphic} if there is a bijection $f$ between $X$ and $Y$ that preserves $<$: $x < y$ if and only if $f(x) < f(y)$.
\end{definition}
...
\begin{proof}
    We say that $h$ is an \vocab{attempt} to mean that $h \colon I \to Y$ where $I$ is some initial segment of $X$, and for all $x \in I$ we have that $h(x) = G\qty(\eval{h}_{I_x})$.
...
This is called the \vocab{successor} of $X$, written $X^+$.

For well-orderings $(X, <_X), (Y, <_Y)$, we say that $(Y, <_Y)$ \vocab{extends} $(X, <_X)$ if $X \subseteq Y$, $\eval{<_Y}_{X} =\, <_X$, and $X$ is an initial segment of $Y$.
We say that well-orderings $X_i$ for $i \in I$ are \vocab{nested} if for all $i, j \in I$, either $X_i$ extends $X_j$ or $X_j$ extends $X_i$.
\begin{proposition}
...
\begin{definition}
    ...
    Then, we say that $X$ has \vocab{order type} $\alpha$.
\end{definition}
...
\begin{definition}
    We say that an ordinal $\alpha$ is a \vocab{successor} if there exists $\beta$ such that $\alpha = \beta^+$.
    Otherwise, $\alpha$ is a \vocab{limit}.
\end{definition}
