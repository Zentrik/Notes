...
\begin{proof}
    ...
    We say that $h$ is an \vocab{attempt} to mean that $h \colon I \to Y$ where $I$ is some initial segment of $X$, s.t. $\forall \; x \in I$ we have that $h(x) = G\qty(\eval{h}_{I_x})$ (note $I_x \subseteq I$).
...
\begin{definition}
    We say that an ordinal $\alpha$ is a \vocab{successor} if there exists $\beta$ s.t. $\alpha = \beta^+$.
