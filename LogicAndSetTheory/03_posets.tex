...
\begin{example}
    In $\mathcal{P}(\mathbb{Q})$, $\qty{(-\infty, x) \cap \mathbb{Q} : x \in \mathbb{R}}$ is an uncountable chain.
\end{example}
...
\begin{proof}
    ...
    We define the partial order on $X$ by $(A, R) \leq (A', R')$ if $(A', R')$ extends $(A, R)$, so $\eval{R'}_A = R$ and $A$ is an i.s. of $A'$ for $R'$.
...
\begin{aside}{Aside - Non Examinable}
    ...
    \begin{definition}[Inflationary]
        A function $f : X \to X$, $X$ a poset, is \vocab{inflationary} if $x \leq f(x) \ \forall x \in X$.
    \end{definition}
