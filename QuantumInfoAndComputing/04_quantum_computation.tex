...
The class of languages for which the membership problem has a randomised classical poly-time algorithm that gives the correct answer with probability at least $\frac{2}{3}$ is called $\symsfup{BPP}$, short for \vocab{bounded-error probabilistic poly-time}.
...
A problem in the \vocab{non-deterministic polynomial-time} complexity class $\symsfup{NP}$ is easy to verify, but may be hard to solve.
...
The \vocab{Boolean satisfiability problem} $\symsfup{SAT}$ seeks an assignment of the variables $x_1, \dots, x_n$ such that $f(x_1, \dots, x_n) = 1$.
...
Any $\symsfup{NP}$ problem can be converted into an application of $\symsfup{SAT}$; we say $\symsfup{SAT}$ is \vocab{$\symsfup{NP}$-complete}.
