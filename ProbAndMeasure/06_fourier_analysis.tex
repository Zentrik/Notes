...
Hence, $f \ast g_t \in L^1$ as $f \in L^1$ (proved earlier) and $\norm{f \ast g_t}_1 \leq \norm{f}_1 < \infty$.
Also $f \ast g_t$ is continuous on $\mathbb R^d$ by noting $f \ast g_t(x) = \int_{\mathbb R^d} f(y) g_t(x-y) \dd{y}$ by translation invariance, then using DCT noting $g$ bounded as cts.
