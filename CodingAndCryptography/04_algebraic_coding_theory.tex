...
\end{align*}
The \vocab{symmetric difference} of two sets $A, B$ is defined to be $A \symmdiff B = A\setminus B \cup B \setminus A$, which corresponds to vector addition in $\mathbb F_2^n$.
...
% In a polynomial ring $R[X]$, $\sum_{i=0}^n a_i X^i = 0$ iff each $a_i$ is zero.
