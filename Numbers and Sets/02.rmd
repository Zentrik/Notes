...
We note $3 \cdot 7 \equiv \mod 10$, so $3 \cdot 7 x \equiv 3 \cdot 2 \mod 10$ so $x \equiv 6 \mod 10$.
:::
...
\end{align*} 
Note $(a', b') = 1$. \
So if $(a, n) = d > 1$, the congruence $ax \equiv b \mod n$ has no solutions unless $d \mid b$, in which case the solutions are exactly those of $\frac{a}{d}x = \frac{b}{d} \mod \frac{n}{d}$.
...
We denote by $\phi(m)$ the number of integers $a$ s.t. $a \leq a \leq m$ s.t $(a, m) = 1$, that is, the number of units modulo $m$.
