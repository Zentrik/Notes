...
\begin{align*}
    ...
    \text{so } &r_n \mid r_i \; \forall \; i = 1, \dots n - 1 \text{ (by induction)}
\end{align*} 
...
Then $a u_1, a u_2, \dots, a u_{\phi(m)}$ are all distinct and invertible modulo $m$ (since $a$ and $u_i$ are units), and hence they are $u_1, u_2, \dots u_{\phi(m)}$ is some order.
