...
::: {.corollary name="Bezout's Theorem"}
...
*Answer*
If there is a solution, then $2a^2 \equiv \mod 3$, but $2 \cdot 0^2 \equiv 0$, $2 \cdot 1 ^2 \equiv 2$, $2 \cdot 2^2 \equiv 2 \mod 3$.
...
\begin{align*}
    ...
    &\iff ax \equiv \mod n \text{ for some } x \in \mathbb{Z}.
\end{align*} 
:::
...
\begin{align*}
    7x &\equiv 4 \mod 30
    \iff 13 \cdot 7 x \equiv 13 \cdot 4 \mod 30 \text{we have $\Longleftarrow$ since $13$ is a unit modulo $30$} \\
    x &\equiv 22 \mod 30
\end{align*}
:::
...
Not $sm \equiv 1 \mod n$ as $tn \equiv 0 \mod n$ and $tn \equiv 1 \mod m$ as $sm \equiv 0 \mod m$. \
...
\begin{align*}
    ...
    &\iff y \equiv \mod m \text{ and } y \equiv x \mod n \\
...
::: {.remark}
Theorem \@ref(thm:twelve) can be extended by induction, to more than two moduli: if $m_1, m_2, \ldots, m_k$ are pairwise coprime, then $\forall \; a_1, a_2, \ldots, a_k \in \mathbb{Z},\ \exists\; x \in \mathbb{Z}$ s.t.\begin{align*}
    x &\equiv a_1 \mod m_1 \\
    x &\equiv a_2 \mod m_2 \\
    &\;\; \vdots \\
    x &\equiv a_k \mod m_k
...
::: {.example}
Modulo $7$, $2^1 \equiv 2$, $2^2 \equiv 4$, $2^3 \equiv 1$, $2^4 \equiv 2$ then repeat $4, 1, 2, 4, 1, 2, \ldots$. \
...
::: {.proof}
If $a \neq 0 \mod p$, then $a$ is a unit mod $p$. \
Thus $ax \equiv ay \mod p$ iff $x \equiv y \mod p$ (multiply by a to go from right to left, and by $a^{-1}$ for converse). 
Hence the numbers $a, 2a, 3a, \ldots, (p - 1)a$ are pairwise incongruent (distinct) modulo $p$ and $\not\equiv 0 \mod p$, so they are $1, 2, 3, \ldots p - 1$ in some order.

Hence
\begin{align*}
    a \cdot 2a \cdot 3a \cdot \ldots \cdot (p - 1)a &\equiv 1 \cdot 2 \cdot 3 \ldots \cdot (p - 1) \mod p \\
    a^{p-1} (p - 1)! &= (p - 1)! \mod p. \\
\end{align*} 
...
Label the elements $u_1, u_2, \ldots u_{\phi(m)}$.
Then $a u_1, a u_2, \ldots, a u_{\phi(m)}$ are all distinct and invertible modulo $m$ (since $a$ and $u_i$ are units), and hence they are $u_1, u_2, \ldots u_{\phi(m)}$ is some order.

It follows that
\begin{align*}
    a u_1 \cdot a u_2 \cdot \ldots \cdot a u_{\phi(m)} &\equiv u_1 \cdot u_2 \cdot \ldots u_{\phi(m)} \mod m,
    \text{that is, }a^{\phi(m)} z &= z \mod p. \\
\end{align*} 
that is $a^{\phi(m)} z = z \mod m$ where $z = u_1 \cdot u_2 \cdot \ldots u_{\phi(m)}$ is a product of units modulo $m$, therefore it itself is a unit.
So we may cancel it to obtain $a^{\phi(m)} = 1 \mod m$ 
:::

::: {.example}
Where $p = 5,\ 4! = 24 \equiv \mod 5$.\
$p = 5,\ 4! = 24 \equiv \mod 5$.
\begin{align*}
...
::: {.proof}
\begin{align*}
    x^2 \equiv \mod p &\iff x^2 - 1 \equiv 0 \mod p \\
...
[E.g. when $p = 1$, $1, 10$ are self inverses and the pairs are $(2, 6),\ (3, 4),\ (5, 9),\ (7, 8)$].\
...
\begin{align*}
    -1 \equiv (p - 1)! &\equiv 1 \cdot 2 \cdot 3 \cdot \ldots \cdot \frac{p - 1}{2} \cdot - \frac{p - 1}{2} \cdot \ldots -3 \cdot -2 \cdot -1 \mod p \\
    &\equiv (-1)^{\frac{p-1}{2}} \left( \left(\frac{p-1}{2} \right)! \right)^2 \mod p \\
\end{align*} ($p-2 \equiv -1 \mod p$)
...
Let us agree to write messages as sequences of numbers, e.g. $A \to 00,\ B \to 1, \ldots, Z \to 25, ! \to 26, \ldots$
...
I think of two large[on the order of a hundred digits] prime $p, q$^.
